...
//g++ -std=c++14 shaders.c assignment2.cpp -I./glm -lglut -lGLU -lGL -lm
...
typedef struct {
    ...
    bool useShaders;
    bool usePixel;
    bool positionalLight;
    bool smoothShading;
    bool usePhong;
    bool tessChanged;
    bool vbo;
} Global;
Global g = { grid, false, 0.0, 0.0, line, true, true, false, false, 0, 0, 8, 2, 0, 0.0, 1.0, 0, false, false, false, false, false, true, true, false, true, false };
...
void init(void)
{
    ...
    shaderProgram = getShader("shaderVert.vert", "shaderVert.frag");
...
void calcGrid(int tess) {
    ...
    for (j = 0; j <= tess; j++) {
        for (i = 0; i <= tess; i++) {
            ...
            vertices[j * (tess + 1) + i].r.x = rEC[0];
            vertices[j * (tess + 1) + i].r.y = rEC[1];
            vertices[j * (tess + 1) + i].r.z = rEC[2];

            vertices[j * (tess + 1) + i].n.x = nEC[0];
            vertices[j * (tess + 1) + i].n.y = nEC[1];
            vertices[j * (tess + 1) + i].n.z = nEC[2];
        }
    }
...
// #define VER
    ...
    for (j = 0; j <= tess; j++) {
        for (i = 0; i <= tess; i++) {
            printf("(%5.2f, %5.2f, %5.2f)", vertices[j * (tess + 1) + i].n.x, vertices[j * (tess + 1) + i].n.y, vertices[j * (tess + 1) + i].n.z);
        }
        printf("\n");
    }
#endif
    ...
    // Indices
    unsigned *pIndices = indices;
    for (j = 0; j < tess; j++) {
        for (i = 0; i <= tess; i++) {
            *pIndices++ = j * (tess + 1) + i;
            *pIndices++ = (j + 1) * (tess + 1) + i;
        }
    }
...
void drawGridVBOs(int tess) {
    ...
    for (int j = 0; j < tess; j++) {
        glDrawElements(GL_QUAD_STRIP, (tess + 1) * 2, GL_UNSIGNED_INT, BUFFER_OFFSET(j * (tess + 1) * 2 * sizeof(unsigned)));
    }
...
void drawGridImmediate(int tess) {
    float stepSize = 2.0 / tess;
    glm::vec3 r, n, rEC, nEC;
    int i, j;
...
void drawSineWaveVBOs(int tess) {
    if (g.tessChanged)
        calcSineWave(tess);
    if (g.animate)
        updateSineWave(tess);
...
void drawSineWaveImmediate(int tess) {
    const float A1 = 0.25, k1 = 2.0 * M_PI, w1 = 0.25;
    const float A2 = 0.25, k2 = 2.0 * M_PI, w2 = 0.25;
    float stepSize = 2.0 / tess;
    glm::vec3 r, n, rEC, nEC;
...
void drawSineWave(int tess)
{
    ...
    // Sine wave
    if (g.vbo)
        drawSineWaveVBOs(tess);
    else
        drawSineWaveImmediate(tess);
...
void keyboard(unsigned char key, int x, int y)
{
    switch (key) {
        ...
        case 'g':
            ...
            //per (vertex, pixel) lighting
        case 'p':
            g.usePixel = !g.usePixel;
            if (g.usePixel && g.usePhong)
                shaderProgram = getShader("shaderPixelPhong.vert", "shaderPixelPhong.frag");
            else if (g.usePixel && !g.usePhong)
                shaderProgram = getShader("shaderPixel.vert", "shaderPixel.frag");
            else if (!g.usePixel && g.usePhong)
                shaderProgram = getShader("shaderVertPhong.vert", "shaderVertPhong.frag");
            else
                shaderProgram = getShader("shaderVert.vert", "shaderVert.frag");
            glutPostRedisplay();
            break;
        ...
        case 'h':
            ...
            //pos or dir light
        case 'd':
            g.positionalLight = !g.positionalLight;
            glutPostRedisplay();
            break;

            //smooth/flat shading
        case 'b':
            g.smoothShading = !g.smoothShading;
            glutPostRedisplay();
            break;
            
            //blinn-phong or phong
        case 'm':
            g.usePhong = !g.usePhong;
            if (g.usePixel && g.usePhong)
                shaderProgram = getShader("shaderPixelPhong.vert", "shaderPixelPhong.frag");
            else if (g.usePixel && !g.usePhong)
                shaderProgram = getShader("shaderPixel.vert", "shaderPixel.frag");
